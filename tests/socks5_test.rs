...
#[test]
#[timeout(8000)]
fn test_socks5_client_half_close_forwards_fin_to_remote() {
    let _guard = test_lock();
    init_log();
    ...
    let backend_thread = std::thread::spawn(move || {
        let listener = TcpListener::bind("127.0.0.1:32352").unwrap();
        listener.set_nonblocking(true).unwrap();
        while !lx2.load(Ordering::SeqCst) {
            match listener.accept() {
                Ok((mut stream, _)) => {
    ...
    // CONNECT 127.0.0.1:32352
    let req = [0x05, 0x01, 0x00, 0x01, 127, 0, 0, 1, 126, 96];
    client.write_all(&req).unwrap();
    let mut conn_resp_head = [0u8; 4];
    client.read_exact(&mut conn_resp_head).unwrap();
    assert_eq!(conn_resp_head[0], 0x05);
    ...
    client.write_all(b"request-data").unwrap();
    client.shutdown(Shutdown::Write).unwrap();

    // Backend sends response only after it receives EOF from proxy.
    let mut out = [0u8; 4];
